...
CKEDITOR.dialog.add('cellProperties', function (editor) {
		...
		items = [
			...
			},
			{
				...
				setup: setupCells(function (element) {
					...
					return textAlignStyle || alignAttr || 'left';
				}),
	...
	function getCellSizeFieldDefinition(fieldName) {
		return {
			...
			children: [{
				...
				setup: setupCells(function (element) {
					// 첫글자 대문자
					var attr = 'offset' + fieldName.charAt(0).toUpperCase() + fieldName.slice(1);
				...
				style: 'display:none',
				labelStyle: 'visibility:hidden;display:none;width:0;overflow:hidden',
				'default': 'px',
				items: [
	...
	function toHex(color) {
		...
		if (typeof color == 'string' && color[0] != '#') {
			...
			color.replace(/(\s*)/g, '');
			color = color.substring(start, end).split(',');
		}
